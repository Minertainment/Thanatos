...
public class StopCommand extends AthenaBungeeCommand {

    public StopCommand() {
        super("stop", "/stop [slave]", "Stop a server.", new Permission("thanatos.stop"));
    ...
    public void onCommand(CommandSender sender, CommandContext args) throws CommandException {
        if(args.argsLength() > 0 && args.getString(0).equalsIgnoreCase("all")) {
            sender.sendMessage(ChatColor.YELLOW + "Executing shutdown procedure for all servers...");
        ...
        if(slave.getStatus() == SlaveStatus.ONLINE) {
            sender.sendMessage(ChatColor.YELLOW + "Executing shutdown procedure on '" + slave.getServerId() + "'...");
        } else {
